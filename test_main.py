import pytest
from main import to_dot, json_patch_to_pymongo, JsonPatch2PyMongoException
...
def test_jp2pym_single_add():
    ...
    assert expected == json_patch_to_pymongo(
        patches), 'should work with single add'
...
def test_jp2pym_escape_characters():
    ...
    assert expected == json_patch_to_pymongo(
        patches), 'should work with escaped characters'
...
def test_jp2pym_array_set():
    ...
    assert expected == json_patch_to_pymongo(
        patches), 'should work with array set'
...
def test_jp2pym_multiple_set():
    ...
    expected = {
        '$push': {'name': {'$each': ['dave', 'john', 'bob'], '$position': 1}}}
    assert expected == json_patch_to_pymongo(
        patches), 'should work with multiple set'
...
def test_jp2pym_multiple_adds_in_reverse_position():
    ...
    expected = {
        '$push': {'name': {'$each': ['john', 'bob', 'dave'], '$position': 1}}}
    assert expected == json_patch_to_pymongo(
        patches), 'should work with multiple adds in reverse position'
...
def test_jp2pym_multiple_adds_last():
    ...
    assert expected == json_patch_to_pymongo(
        patches), 'should work with multiple adds'
...
def test_jp2pym_multiple_adds_last_with_nulls():
    ...
    assert expected == json_patch_to_pymongo(
        patches), 'should work with multiple adds with some null at the end'
...
def test_jp2pym_multiple_adds_last_with_nulls_with_position():
    ...
    expected = {
        '$push': {'name': {'$each': [None, 'bob', None], '$position': 1}}}
    assert expected == json_patch_to_pymongo(
        patches), 'should work with multiple adds with some null and position'
...
def test_jp2pym_remove():
    ...
    assert expected == json_patch_to_pymongo(
        patches), 'should work with remove'
...
def test_jp2pym_replace():
    ...
    assert expected == json_patch_to_pymongo(
        patches), 'should work with replace'
...
def test_jp2pym_add_treated_as_replace():
    ...
    expected = {'$set': {'name': 'dave'}}
    assert expected == json_patch_to_pymongo(
        patches), 'should be treated as replace if adding without position'
